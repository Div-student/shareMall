...
> 版本：v1.0　|　配套：《数据模型与接口字段清单.md》
>
> 约定：引擎 InnoDB，字符集 utf8mb4，金额统一 `decimal(12,2)`，所有表含 `created_at`/`updated_at`，可软删表含 `deleted_at`。敏感字段（身份证、收款账户）应用层加密后存储。进入开发时可将下方 SQL 导出为 `server/migrations/schema.sql`。

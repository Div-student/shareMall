...
> 设计要点：三类资产同表分列，便于一致性；所有变更必须写 `fund_record` 流水，余额变更与流水在同一事务内。
...
| change_type | enum | order_accrue/checkin_cashout/order_deduct/nft_exchange/nft_trade_income/aftersale_void/aftersale_rollback/withdraw/task_reward |
...
| stock | int | 兑换剩余库存 |
| exchange_fund | decimal | 兑换所需可用贡献金 |
...
> 关键配置键示例：`fund.default_ratio`、`fund.tiers`、`fund.checkin_days`、`fund.miss_rule`、`fund.deduct_limit_rate`、`nft.market_fee_rate`、`withdraw.min/max/fee`、`invite.required`、`sms.provider`。
...
| 贡献金流水 | GET | `/api/fund/records` | assetType, changeType, page |
...
| 接口 | 方法 | 路径 | 请求/响应要点 |
| --- | --- | --- | --- |
| 藏品商城列表 | GET | `/api/nft` | 返回 exchangeFund, stock |
| 藏品详情 | GET | `/api/nft/:id` | rightsDesc, 参考市场价 |
| 兑换藏品 | POST | `/api/nft/:id/exchange` | — → 校验并扣减可用贡献金 |
| 我的藏品 | GET | `/api/nft/mine` | 含 status(holding/listed) |
| 挂单卖出 | POST | `/api/nft/listings` | userNftId, price → 返回手续费与预计到账（提现金） |
| 撤单/改价 | DELETE/PUT | `/api/nft/listings/:id` | price |
| 二级市场列表 | GET | `/api/nft/trade` | 在售挂单 |
| 购买挂单 | POST | `/api/nft/trade/:listingId/buy` | → 成交，卖家所得入提现金 |
| 交易记录 | GET | `/api/nft/trade/records` | type, page |
...
| 藏品发行 | POST | `/api/admin/nft` | name, supply, exchangeFund, limit |
| 二级市场配置 | GET/PUT | `/api/admin/nft/market` | enabled, feeRate, priceRange |
...
- 任何资产变更（pending/available/withdrawable）必须：扣减/增加余额 + 写 `fund_record`，同一数据库事务内完成。
- 下单使用抵扣：校验 `available_fund` 充足 → 扣减 → 写流水 → 订单记录 `fund_deduct_amount`。
- 支付成功回调：幂等处理 → 累计 `pending_fund`（`accrued_fund`）→ 写流水。
- 打卡签到：校验当天未签 → 计划 `signed_days/cashed_amount` 更新 + `available_fund` 增加 + 写流水；漏卡由定时任务结算 `void_amount`。
- 藏品成交：转移 `user_nft` 归属 → 卖家 `withdrawable_cash` 增加（price - fee）→ 写流水 → 记 `nft_trade`。
- 提现：申请时冻结（`withdrawable_cash` 转 `frozen_cash`）→ 审核通过打款成功后扣减冻结；驳回/失败则解冻退回。
- 售后退款：按规则回退已抵扣可用贡献金 + 冲销已累计待兑现贡献金，均写流水。

...
| id | bigint PK | |
| name | string | 券名称 |
| type | enum | fixed（满减）/ discount（折扣，value 为系数如 0.9） |
| value | decimal | 面额或折扣系数 |
| min_amount | decimal | 使用门槛（商品总额） |
| total | int | 发放总量，0=不限 |
| claimed / used | int | 已领取 / 已核销计数 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | bigint PK | 下单 preview/create 的 `couponId` 即此 id |
| user_id | bigint FK | |
| coupon_id | bigint FK | |
| status | enum | unused / used / expired |
| order_id | bigint | 核销订单 |
| claimed_at / used_at | datetime | |

> 完整业务规则见 [优惠券业务说明.md](./优惠券业务说明.md)。

#### campaign / dict
- campaign：营销活动（title、banner、type、status、rule JSON 等）。
- dict：系统字典（group、code、label、sort、status）。
...
| 接口 | 方法 | 路径 | 请求/响应要点 |
| --- | --- | --- | --- |
| 下单预览（试算） | POST | `/api/orders/preview` | items, addressId, useFund(bool), fundAmount, **couponId**（user_coupon.id）→ 返回 totalAmount、fundDeductAmount、**couponAmount**、payAmount、accruedFund |
| 创建订单 | POST | `/api/orders` | items, addressId, useFund, fundAmount, **couponId** → 事务核销券；返回 orderNo |
...
| 接口 | 方法 | 路径 | 请求/响应要点 |
| --- | --- | --- | --- |
| 可领取优惠券 | GET | `/api/coupons/claimable` | 含 claimed 是否已领 |
| 我的优惠券 | GET | `/api/coupons/mine` | list：id、status、coupon 模板 |
| 领取优惠券 | POST | `/api/coupons/:id/claim` | id 为券模板 id |
| 活动列表 | GET | `/api/campaigns` | 生效中活动 |
| 字典 | GET | `/api/dicts/:group` | 如 order_status |
| 客服配置 | GET | `/api/service-config` | phone、faq 等 |
| 后台优惠券 CRUD | GET/POST/PUT | `/api/admin/coupons` | 权限 coupon:manage |
| 后台券上下架 | POST | `/api/admin/coupons/:id/status` | status: enabled/disabled |

> 抵扣计算、与贡献金叠加、核销事务：见 [优惠券业务说明.md](./优惠券业务说明.md)。
...
- **售后**：回退抵扣 + 冲销累计，均写流水。
- **优惠券**：创建订单时校验并核销 `user_coupon`；`payAmount = 商品总额 - 贡献金抵扣 - 优惠券抵扣 + 运费`；详见 [优惠券业务说明.md](./优惠券业务说明.md)。
...
- 优惠券领取、抵扣、核销：见 [优惠券业务说明.md](./优惠券业务说明.md)。
